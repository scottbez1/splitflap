...
REGISTRATION_MARK2 = "M -47,47 L -48,47 L -48,48 L -47,48 z"
...
class SvgProcessor(object):
    ...
    def delete_registration_mark(self):
        for path in self.svg_node.getElementsByTagName('path'):
            ...
            if REGISTRATION_MARK2 not in data.value:
                raise ValueError("Registration mark not found!")
            data.value = data.value.replace(REGISTRATION_MARK, '')
            data.value = data.value.replace(REGISTRATION_MARK2, '')
            if data.value.strip() == '':
                self.svg_node.removeChild(path)
        return len(self.svg_node.getElementsByTagName('path')) > 0
